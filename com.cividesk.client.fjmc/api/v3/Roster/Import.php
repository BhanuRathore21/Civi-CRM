...
*/
  ...
  while ($data = $reader->getNextRow()) {
    ...
    $sql = "SELECT DISTINCT(ccb.id) as contact_id, ccb.first_name,ccb.last_name, club.id as club_contact_id, ccb.is_deceased, ccb.display_name, ccb.id, eprim.email as email, cpl.phone AS landline, cpl.is_primary AS landline_is_primary, cpm.phone AS mobile, cr.is_active
...
function _roster_update($entity, $params, $update) {
  ...
  if (isset($update['email'])) $update['email'] = strtolower($update['email']);
  ...
  if (in_array($entity, array('Phone', 'Email')) && empty($update['email']) && empty($update['phone'])
          && !array_key_exists('is_primary', $update)) {    // This is NOT a primary phone/email update
    if (!empty($current['id'])) {
